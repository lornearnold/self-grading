...
def main():
    parser = ArgumentParser(
        description="Update Canvas grades for a student-graded assignment."
    )
    ...
    parser.add_argument(
        "-p", "--check_problem", help="Check Problem name (e.g. 'Problem 3')"
    )
    parser.add_argument(
        "-f", "--fetch", help="Fetch scores from Canvas", action="store_true"
    )
    parser.add_argument(
        "-u", "--upload", help="Upload scores to Canvas", action="store_true"
    )
    parser.add_argument(
        "-o",
        "--overwrite",
        help="Overwrite existing instructor scores",
        action="store_true",
    )
    parser.add_argument(
    ...
    if args.course_id:
        ...
        if course_name == "":
            ...
            matching_courses = [
                c for c in canvas.get_courses() if course_name.lower() in c.name.lower()
            ]
    ...
    try:
        a_submit = course.get_assignments(
            search_term=assignment_name + " " + config["submission_suffix"]
        )[0]
    ...
    try:
        a_self = course.get_assignments(
            search_term=assignment_name + " " + config["score_suffix"]
        )[0]
    ...
    if args.fetch:
        ...
        p_point_vals = {
            q.id: q.points_possible
            for q in q_probs
            if config["problem_prefix"] in q.question_name
        }
        p_names = {
        ...
        if os.path.exists(f"{assignment_name}_scores.csv"):
            ...
            p_cols = list(
                chain.from_iterable([[p + "_s", p + "_i"] for p in p_names.values()])
            )
            cols = (
                ["name", "hw_id", "grade_id", "checked"]
                + p_cols
                + ["student_score", "final_score", "error"]
            )
        ...
        if not args.check_problem:
            args.check_problem = input(
                'Enter the Check Problem name (e.g. "Problem 3"): '
            )
        ...
        if check_problem_id is None:
            print(
                "No matching Check Problem found. Please check the name and try again."
            )
            return
        ...
        for user in course.get_users(enrollment_type=["student"]):
            ...
            if pd.notnull(df_scores.loc[user.id, "hw_id"]):
                if hw_sub.id != df_scores.loc[user.id, "hw_id"]:
                    ...
                    if args.verbose:
                        print(" * New HW submission found")

                    overwrite = args.overwrite
                    if not args.overwrite:  # Prompt for overwrite
                        clear_scores = input(
                            f" * Clear ALL instructor scores for {user.name}? (y/n) "
                        )
                    ...
                    if overwrite:
                        ...
                        df_scores.loc[user.id, df_scores.columns.str.endswith("_i")] = (
                            None
                        )
            ...
            if inst_evaluation.grade is not None:
                df_scores.loc[user.id, p_names[check_problem_id] + "_i"] = float(
                    inst_evaluation.grade
                )
            else:
            ...
            grade_sub = a_self.get_submission(
                user=user.id, include=["submission_history"]
            )
            ...
            if pd.notnull(df_scores.loc[user.id, "grade_id"]):
                if s_latest["id"] != df_scores.loc[user.id, "grade_id"]:
                    if args.verbose:
                        print(" * New student grade submission found.")
            ...
            for q in s_latest["submission_data"]:  # Loop over quiz questions
                if q["question_id"] in p_point_vals:  # If Problem
                    ...
                    if student_score > p_point_vals[q["question_id"]]:
                        log_error(
                            f"Student entered score greater than maximum for {p_name}."
                        )
                        continue
                    if student_score < 0:
                        log_error(
                            f"Student entered a score less than zero for {p_name}."
                        )
                        continue
    ...
    if args.upload:
        ...
        try:
            a_final = course.get_assignments(
                search_term=assignment_name + " " + config["final_suffix"]
            )[0]
            ...
            if create_final.lower() == "y":
                ...
                print(
                    a_final.name,
                    "created. You must publish it on Canvas before proceeding.",
                )
            else:
                print("Exiting.")
